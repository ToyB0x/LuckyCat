...
export async function diagnoseGoogleCloud(
  env: DebugSettings,
  project: string,
  ...
  try {
    ...
    authenticationReason = error instanceof CredentialError ? error.code : 'authentication_failed';
  }
  ...
  try {
    for (const source of ['disks', 'addresses'] as SourceKind[]) {
      if (token)
        sources.push(await collectInventory(project, source, token, controller.signal, fetcher));
      else
        sources.push({
          source,
          ...
          scopes: [],
          records: [],
          issues: [{ reason: 'authentication_failed' }],
        });
    }
  } finally {
    clearTimeout(timer);
  ...
  return {
    ...
    status: rules.every((r) => r.status === 'evaluated')
      ? 'evaluated'
      : rules.some((r) => r.evaluatedResources > 0 || r.status === 'evaluated')
        ? 'partially_evaluated'
        : 'not_evaluated',
    sources: sources.map(({ records, ...source }) => ({
      ...source,
