...
export function allowedProjects(env: DebugSettings): string[] {
  return [...new Set((env.GOOGLE_DEBUG_PROJECTS ?? '').split(',').map(p => p.trim()).filter(Boolean))]
...
export async function checkGoogleConnection(env: DebugSettings, project: string, fetcher: typeof fetch = fetch) {
  ...
  try {
    token = await createLocalServiceAccountProvider({
      mode: env.GOOGLE_AUTH_MODE, serviceAccountJson: env.GOOGLE_SERVICE_ACCOUNT_JSON, fetcher,
    }).getAccessToken();
  } catch (error) {
    return { project, checkedAt, authentication: 'failed', reason: error instanceof CredentialError ? error.code : 'authentication_failed', sources: [], diagnosis: 'not_run' };
  }
  const sources = [];
  ...
  for (const source of ['disks', 'addresses', 'instances'] as const) {
    try {
      ...
      let status: string;
      if (!response.ok) {
        status = response.status === 401 ? 'authentication_rejected'
          : response.status === 403 ? 'access_denied_or_api_disabled'
          : response.status === 429 ? 'quota_limited' : 'retrieval_failed';
      } else {
        const data: unknown = await response.json();
        if (!data || typeof data !== 'object' || Array.isArray(data)) throw new Error();
        // Warnings require further inspection; do not claim full access from HTTP 200.
        status = hasIncompleteCoverage(data as Record<string, unknown>) ? 'read_probe_incomplete' : 'read_probe_succeeded';
      }
...
function hasIncompleteCoverage(data: Record<string, unknown>): boolean {
  if (data.unreachables !== undefined && (!Array.isArray(data.unreachables) || data.unreachables.length > 0)) return true;
  const warningNeedsReview = (warning: unknown) => warning !== undefined &&
    (!warning || typeof warning !== 'object' || !('code' in warning) || warning.code !== 'NO_RESULTS_ON_PAGE');
  ...
  return Object.values(data.items).some(value => !value || typeof value !== 'object' ||
