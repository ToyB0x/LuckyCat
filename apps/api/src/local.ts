...
export default {
  async fetch(request: Request, env: LocalEnv): Promise<Response> {
    ...
    if (origin && ![url.origin, 'http://127.0.0.1:3000', 'http://localhost:3000'].includes(origin))
      return reply({ error: 'origin_denied' }, 403);
    if (request.method === 'GET' && url.pathname === '/local-debug/status') {
      return reply({
        enabled: debugEnabled(env),
        credentialConfigured: Boolean(env.GOOGLE_SERVICE_ACCOUNT_JSON),
        projects: allowedProjects(env),
      });
    }
    if (url.pathname === '/local-debug/history' || url.pathname.startsWith('/local-debug/history/'))
    ...
    try {
      if (request.method === 'GET' && url.pathname.startsWith('/local-debug/workflow/')) {
        ...
        return reply({
          status: state.status,
          ...(state.status === 'complete' ? { output: state.output } : {}),
        });
      }
      if (
        request.method !== 'POST' ||
        !['/local-debug/check', '/local-debug/workflow', '/local-debug/diagnose'].includes(
          url.pathname,
        )
      )
      ...
      if (
        ...
        Object.keys(body).some((key) => key !== 'project') ||
        !allowedProjects(env).includes(body.project)
      )
        return reply({ error: 'project_not_allowed' }, 400);
      if (url.pathname === '/local-debug/diagnose')
        return reply(await diagnoseGoogleCloud(env, body.project));
      if (url.pathname === '/local-debug/check')
        return reply(await checkGoogleConnection(env, body.project));
      const instance = await env.GOOGLE_CONNECTION_CHECK.create({
