...
const env = {
  ...
  GOOGLE_DEBUG_PROJECTS: 'debug-example',
  GOOGLE_SERVICE_ACCOUNT_JSON: JSON.stringify({
    type: 'service_account',
    client_email: 'reader@debug-example.iam.gserviceaccount.com',
...
test('no network request before explicit mode, valid credentials and allowed project', async () => {
  ...
  await assert.rejects(checkGoogleConnection({}, 'debug-example', noNetwork), {
    message: 'debug_disabled',
  });
  await assert.rejects(checkGoogleConnection(env, 'other-project', noNetwork), {
    message: 'project_not_allowed',
  });
  const result = await checkGoogleConnection(
    { ...env, GOOGLE_SERVICE_ACCOUNT_JSON: 'secret' },
    'debug-example',
    noNetwork,
  );
...
test('separates authentication and bounded read probes, preserving partial failure', async () => {
  ...
  const result = await checkGoogleConnection(env, 'debug-example', async (url, init) => {
    ...
    assert.equal(new URL(url).searchParams.get('maxResults'), '1');
    assert.ok(!init.method || init.method === 'GET');
    if (url.includes('/disks?')) return Response.json({ items: { 'zones/example': {} } });
    if (url.includes('/addresses?'))
      return new Response('sensitive-provider-error', { status: 403 });
  ...
  assert.deepEqual(
    ...
    ['read_probe_succeeded', 'access_denied_or_api_disabled', 'read_probe_incomplete'],
  );
  assert.ok(!JSON.stringify(result).includes('secret-test-token'));
  assert.ok(!JSON.stringify(result).includes('sensitive-provider-error'));
});
...
test('quotas, malformed response and invalidated credentials remain failures, never zero findings', async () => {
  ...
  assert.deepEqual(
    result.sources.map((s) => s.status),
    ['quota_limited', 'retrieval_failed', 'authentication_rejected'],
  );
  assert.equal(result.diagnosis, 'not_run');
});
...
test('regular Worker entrypoint does not import local debug code', async () => {
  ...
  assert.ok(!main.includes('./local'));
  assert.ok(!main.includes('./debug'));
});
...
test('redirected read probes remain failures without forwarding the access token', async () => {
  ...
  assert.deepEqual(
    result.sources.map((s) => s.status),
    ['retrieval_failed', 'retrieval_failed', 'retrieval_failed'],
  );
  assert.equal(result.diagnosis, 'not_run');
});
