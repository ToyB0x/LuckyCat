...
test('authentication and allowlist gates prevent collection; failed authentication leaves both rules unevaluated', async () => {
  ...
  await assert.rejects(diagnoseGoogleCloud({}, 'debug-example', noNetwork), {
    message: 'debug_disabled',
  });
  await assert.rejects(diagnoseGoogleCloud(env, 'other-project', noNetwork), {
    message: 'project_not_allowed',
  });
  const result = await diagnoseGoogleCloud(
  ...
  assert.ok(
    result.rules.every((r) => r.status === 'not_evaluated' && r.conclusion === 'incomplete'),
  );
});
test('end-to-end JSON keeps one source failure separate from valid findings and never includes tokens or raw inventory', async () => {
  ...
  const result = await diagnoseGoogleCloud(env, 'debug-example', async (url) => {
    ...
    if (url === 'https://oauth2.googleapis.com/token')
      return Response.json({
        access_token: 'do-not-expose-token',
        token_type: 'Bearer',
        expires_in: 3600,
      });
      ...
      items: {
        'regions/region-a': {
          addresses: [
            {
              selfLink:
                'https://www.googleapis.com/compute/v1/projects/debug-example/regions/region-a/addresses/address-a',
              status: 'RESERVED',
  ...
  assert.equal(result.rules[0].conclusion, 'incomplete');
  assert.equal(result.rules[1].candidates.length, 1);
  assert.equal(result.schemaVersion, '2');
  assert.deepEqual(result.rules[1].candidates[0].amount, {
    status: 'estimated',
  ...
  for (const secret of ['do-not-expose', '203.0.113.10', 'private_key'])
