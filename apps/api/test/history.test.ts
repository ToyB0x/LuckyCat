...
beforeAll(async () => {
  ...
  for (const statement of (
    await readFile(
      new URL('../../../packages/db/migrations/0001_diagnostic_history.sql', import.meta.url),
      'utf8',
    )
  )
    .split(';')
  ...
  env = {
    LOCAL_DIAGNOSTICS: db as unknown as D1Database,
    GOOGLE_DEBUG_PROJECTS: 'history-example',
  };
});
afterAll(() => mf.dispose());
const call = (path = '', init?: RequestInit, settings = env) =>
  handleHistory(new Request(`http://localhost/local-debug/history${path}`, init), settings);
function fixture(): DiagnosticResult {
  const at = '2026-09-08T00:00:00.000Z';
  const disk: Inventory = {
    source: 'disks',
    status: 'partial',
    startedAt: at,
    completedAt: at,
    pages: 1,
    scopes: ['zones/example-a'],
    issues: [{ reason: 'unreachable_scope', scope: 'zones/example-b' }],
    records: [
      {
        resource: 'projects/history-example/zones/example-a/disks/demo-disk',
        observedAt: at,
        state: 'READY',
        referenceCount: 0,
  ...
  return {
    ...
    limits: {
      ...
      bytesPerResponse: 2097152,
      requestMs: 10000,
      totalMs: 30000,
    },
    status: 'partially_evaluated',
    sources: [disk, address].map(({ records, ...source }) => ({
      ...source,
      receivedResources: records.length,
    })),
    rules: [disk, address].map(evaluateInventory),
  };
}
const post = (id: string, result: unknown) =>
  call('', {
    method: 'POST',
...
test('D1 round trip keeps partial coverage and unknown prices, strips extra fields, and supports idempotent saves and deletion without keys', async () => {
  ...
  const raw = await env
    .LOCAL_DIAGNOSTICS!.prepare('SELECT result_json FROM diagnostic_history WHERE id = ?')
    .bind(id)
    .first<{ result_json: string }>();
  assert.ok(!raw!.result_json.includes('synthetic-do-not-store'));
  const changed = fixture();
  changed.completedAt = '2026-09-08T00:01:00.000Z';
  assert.equal((await post(id, changed)).status, 409);
  assert.equal(
    (await call(`/${id}`, { method: 'DELETE' }, { LOCAL_DIAGNOSTICS: env.LOCAL_DIAGNOSTICS }))
      .status,
    200,
  );
...
test('invalid input, oversized requests and missing storage are errors rather than empty history', async () => {
  ...
  assert.equal(
    (await post(crypto.randomUUID(), { ...fixture(), project: 'other-example' })).status,
    403,
  );
  assert.equal(
    (await post(crypto.randomUUID(), { ...fixture(), padding: 'x'.repeat(1024 * 1024) })).status,
    413,
  );
...
test('unsupported saved snapshots can be listed and deleted, but never rendered as no matches', async () => {
  const id = crypto.randomUUID();
  await post(id, fixture());
  await env
    .LOCAL_DIAGNOSTICS!.prepare('UPDATE diagnostic_history SET result_json = ? WHERE id = ?')
    .bind('{"schemaVersion":"future"}', id)
    .run();
