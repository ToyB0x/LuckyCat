...
JSONには候補のリソース識別子と必要な根拠だけを含め、IPアドレス、ラベル、説明、認証情報、Googleの生エラーは含めません。結果はローカル画面で確認し、Git管理対象ファイルへ保存しないでください。診断結果のローカルD1保存は明示的な操作で行い、Workflowへの診断結果保存やリソースの自動変更は行いません。起動や設定読み込みだけで診断は始まりません。
...
開発専用・顧客向け未提供です。初回に`pnpm --filter @luckycat/api db:migrate:local`を実行し、通常どおり`pnpm dev`で起動します。ローカル検証画面で診断後に「ローカルに保存」を押します。保存は任意で、失敗しても診断結果は画面に残ります。保存対象のプロジェクトはローカル設定の許可リストに含めてください。

`/dev/history`で保存日時の新しい順に20件ずつ確認し、履歴を開くと実診断と同じ結果表示で根拠を確認できます。部分取得・未評価・金額不明も保持します。金額と根拠は診断時点のスナップショットであり、閲覧時に再取得・再評価・再計算しません。保存日時と診断日時を区別します。削除は保存した履歴だけを対象とし、Google Cloudのリソースを変更しません。

履歴の閲覧・削除にはローカルAPIとD1が必要ですが、Google Cloudの鍵は不要です。これらの操作からクラウド接続は始まりません。`/dev/diagnostics`の架空データプレビューはAPI・D1なしで引き続き利用でき、履歴へ自動保存しません。

保存するのは現在の結果仕様で検証したフィールドだけです。1件の保存リクエストは1MiBまでとし、対応しない形式や壊れた結果を「該当なし」に変換しません。履歴APIはローカル設定に限定し、本番ビルドには含めません。ローカルD1はGit管理対象外の`apps/api/.wrangler/`に置き、明示的に削除するまで保持します。自動保持期限や本番のテナント認可・削除保証は今回の範囲外です。ローカルDBやバックアップを公開・共有しないでください。

D1の[ローカル開発](https://developers.cloudflare.com/d1/best-practices/local-development/)と[マイグレーション](https://developers.cloudflare.com/d1/reference/migrations/)に従い、セットアップはローカルだけに適用します。
