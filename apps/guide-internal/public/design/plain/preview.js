...
(() => {
  ...
  const scenarios = [
    ['findings', '候補あり', 'Findings'], ['many', '候補50件', '50 findings'],
    ['empty', '該当なし', 'No matches'], ['partial', '部分取得', 'Partial collection'],
    ['unknown', '金額不明', 'Unknown amounts'], ['auth', '認証失敗', 'Authentication failed'],
    ['error', '通信失敗', 'Transport failed'], ['loading', '読み込み中', 'Loading'],
  ];
  // Fixed presentation data only; amounts are not calculated from cloud resources.
  const sample = [
    { name: 'demo-cache-disk', kind: 'disk', type: 'pd-standard', capacity: '200 GiB', value: 8, basis: '$0.04 × 200 GiB', zone: 'asia-northeast1-a' },
    { name: 'demo-reserved-ip', kind: 'ip', value: 7.3, basis: '$0.01 × 730 h', zone: 'asia-northeast1' },
    { name: 'demo-review-disk', kind: 'disk', type: 'pd-extreme', capacity: '500 GiB', value: null, basis: null, zone: 'asia-northeast1-a' },
  ];
  const pairs = entries => entries.map(([key, value]) => `<div><dt>${key}</dt><dd>${value}</dd></div>`).join('');
  function candidate(item) {
    ...
    const kind = disk ? l('未接続の永続ディスク', 'Unattached persistent disk') : l('未割り当ての静的外部IPv4', 'Unassigned static external IPv4');
    return `<details class="candidate">
      ...
      <div class="evidence"><p class="resource">${path}</p><dl>${pairs([
        ...
        [l('状態 / 参照先', 'State / references'), `${disk ? 'READY' : 'RESERVED'} / ${l('参照先0件', '0 references')}`],
        [l('ルール / バージョン', 'Rule / version'), `<span class="resource">${rule} / v1</span>`],
        [l('構成', 'Configuration'), disk ? `${item.type} / ${item.capacity}` : 'EXTERNAL / IPV4'],
        [l('連続した未使用期間', 'Continuous unused duration'), l('不明（現在の状態のみ）', 'Unknown (current state only)')],
        [l('概算の内訳（仮）', 'Prototype estimate basis'), item.value === null ? l('この見本では単価未設定のため算出していません。', 'No price is configured in this sample.') : `${item.basis} = ${usd(item.value)} USD/${l('月', 'month')}`],
  ...
  function content() {
    ...
    let rows = scenario === 'empty' ? [] : scenario === 'many'
      ? Array.from({ length: 50 }, (_, index) => ({ ...sample[index % 3], name: `${sample[index % 3].name}-${String(index + 1).padStart(2, '0')}` }))
    ...
    return `${partial ? `<section class="notice warning" role="status"><h2>${l('候補が見つかりましたが、一部を取得できていません', 'Findings available; some scope could not be collected')}</h2><p>${l('ディスクの europe-west1-b はアクセス不可でした。取得・評価できた候補は残しています。下の取得範囲を確認してください。', 'Disk scope europe-west1-b was inaccessible. Valid findings remain visible. Review collection coverage below.')}</p></section>` : ''}
      <dl class="summary">${pairs([
        [l('確認できた候補', 'Observed candidates'), `${rows.length}<small> ${l('件', 'items')}</small>`],
        [l('候補リソースの概算月額・算出分のみ', 'Candidate monthly cost · priced portion'), `<span class="amount">${!rows.length ? '—' : priced.length ? usd(subtotal) : l('金額不明', 'Unknown')}</span>${priced.length ? '<small> USD</small>' : ''}`],
        [l('取得・評価', 'Collection / evaluation'), `<small>${partial ? l('一部未完了', 'Partially complete') : l('対象範囲は完了', 'Complete within scope')}</small>`],
      ])}</dl>
      ...
      ${rows.length ? `<section aria-labelledby="findings-heading"><div class="list-heading"><h2 id="findings-heading">${l('見直し候補', 'Review candidates')}</h2><span class="secondary">${l('行を開くと根拠を確認できます', 'Expand a row to review evidence')}</span></div><div class="candidate-list">${rows.map(candidate).join('')}</div></section>`
  ...
  function render(focusId) {
    ...
    document.getElementById('scenario').addEventListener('change', event => { scenario = event.target.value; render('scenario'); });
    document.getElementById('language').addEventListener('change', event => { language = event.target.value; render('language'); });
