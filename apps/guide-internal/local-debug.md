...
The screen shows the project, run time, candidate list, and evidence for each candidate. Review the resource name, location, observation time, and condition. Unevaluated resources and collection gaps appear separately with reasons. Price estimation is currently a prototype. The screen shows approximate monthly amounts and a subtotal of priced candidates, with rates and quantities in the details. Unused duration remains unknown. JSON stays available in a collapsed detail section.
