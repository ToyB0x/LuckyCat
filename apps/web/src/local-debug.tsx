...
export function LocalDebugPanel() {
  ...
  async function run(action: Action) {
    ...
    try {
      ...
      const response = await fetch(`/local-debug/${path}`, {
      ...
      if (!response.ok)
        throw new Error(
          `確認できませんでした（HTTP ${response.status}）。ローカルAPIと設定を確認してください。`,
        );
      ...
      if (action === 'diagnose') setDiagnosis(parseDiagnosticResult(data, project));
        ...
        if (
          ...
          !('credentialConfigured' in data) ||
          typeof data.credentialConfigured !== 'boolean' ||
          !('projects' in data) ||
          !Array.isArray(data.projects) ||
          !data.projects.every((p) => typeof p === 'string')
        )
          throw new Error('設定状態の応答を確認できませんでした。');
        ...
        if (
          ...
          throw new Error('Workflowの開始を確認できませんでした。');
        setWorkflow(data.id);
        setAuxiliary({ status: '開始しました。結果を確認してください。' });
      ...
      setError(
        action === 'diagnose' &&
          ...
          failure.message.startsWith('診断結果の形式')
          ? failure.message
          : failure instanceof Error && failure.message.startsWith('確認できませんでした（HTTP')
            ? failure.message
  ...
  return (
    <section className="local-debug" aria-label="Google Cloudのローカル検証">
      <style>{diagnosticStyles}</style>
      <header>
        <p className="eyebrow">Google Cloud · 開発用</p>
        <h2>リソースの見直し候補を確認</h2>
        <p>
          未接続ディスクと未割り当ての静的外部IPv4を、現在の状態から確認します。価格推定は現在仮実装で、固定単価による概算を表示します。
        </p>
      </header>
      <div className="diagnostic-controls">
        <button type="button" className="secondary" onClick={() => void run('status')}>
          設定状態を読み込む
        </button>
        {status && (
          <>
            <label>
              ...
              >
                {status.projects.map((value) => (
                  <option key={value}>{value}</option>
                ))}
              </select>
            </label>
            <p className="muted">
              認証モード：{status.enabled ? '有効' : '無効'} ／ 鍵：
              {status.credentialConfigured ? '設定あり（有効性は未確認）' : '未設定'}
            </p>
            {!ready && (
              <p>許可プロジェクトとサービスアカウントJSON鍵のローカル設定を確認してください。</p>
            )}
            <button
              type="button"
              className="primary"
            ...
            <p className="muted">
              Google Cloudの読み取りクォータを消費します。リソースの変更は行いません。
            </p>
          </>
        )}
      </div>
      ...
      {diagnosis && (
        <>
          <SaveDiagnostic result={diagnosis} />
          <DiagnosticResults result={diagnosis} />
        </>
      )}
      {status && (
        <details className="connection-tools">
          <summary>接続確認ツール</summary>
          <p>
            OIDCサーバーを使わず、サーバー側のJSON鍵で確認します。接続確認は各1ページの読み取りのみで、診断ではありません。
          </p>
          <button
            type="button"
            className="secondary"
      ...
      <p className="muted privacy-note">
        ローカル専用・顧客向け未提供。結果にはリソース識別子が含まれます。公開先へ貼り付けないでください。
      </p>
    </section>
  );
}
