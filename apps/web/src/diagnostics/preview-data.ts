...
export function createPreviewState(scenario: PreviewScenario): PreviewState {
  ...
  const disk: Inventory = {
    ...
    startedAt: at,
    completedAt: at,
    scopes: ['zones/asia-northeast1-a'],
    pages: 1,
    issues: [],
    records: [],
  };
  const address: Inventory = {
    ...disk,
    source: 'addresses',
    scopes: ['regions/asia-northeast1'],
    records: [],
  };
  const count =
    scenario === 'empty' || scenario === 'authentication' ? 0 : scenario === 'many' ? 50 : 3;
  for (let i = 0; i < count; i++) {
    const suffix = scenario === 'many' ? `-${String(i + 1).padStart(2, '0')}` : '';
    if (i % 3 === 1 && scenario !== 'unknown') {
      address.records.push({
        resource: `projects/${project}/regions/asia-northeast1/addresses/demo-reserved-ip${suffix}`,
        observedAt: at,
        state: 'RESERVED',
        referenceCount: 0,
      ...
      disk.records.push({
        resource: `projects/${project}/zones/asia-northeast1-a/disks/${unpriced ? 'demo-review-disk' : 'demo-cache-disk'}${scenario === 'unknown' ? `-${i + 1}` : suffix}`,
        observedAt: at,
        state: 'READY',
        referenceCount: 0,
        referencesOmitted: true,
        diskType: unpriced ? 'pd-extreme' : 'pd-standard',
        sizeGb: unpriced ? '500' : '200',
        replicating: false,
      });
    }
  }
  if (scenario === 'partial') {
    disk.status = 'partial';
    disk.issues = [{ reason: 'unreachable_scope', scope: 'zones/europe-west1-b' }];
  }
  if (scenario === 'authentication')
    for (const source of [disk, address]) {
      source.status = 'not_attempted';
      source.pages = 0;
      source.scopes = [];
      source.issues = [{ reason: 'authentication_failed' }];
    }
