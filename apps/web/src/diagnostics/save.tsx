...
export function SaveDiagnostic({ result }: { result: DiagnosticResult }) {
  ...
  async function save() {
    ...
    setError(false);
    try {
      const value = await historyRequest('', {
        method: 'POST',
  ...
  return (
    <div className="diagnostic-save">
      ...
      {state === 'saved' && (
        <p>
          <output>診断時点の結果を保存しました。</output>
        </p>
      )}
      {error && (
        <p role="alert">
          保存できませんでした。診断結果は画面に残っています。ローカルAPI・D1のセットアップと対象プロジェクトの設定を確認し、再試行してください。
        </p>
      )}
    </div>
  );
