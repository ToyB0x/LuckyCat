...
test('all synthetic results pass the real response boundary; request states have no result', () => {
  for (const scenario of previewScenarios) {
    ...
    if (state.kind === 'result')
      assert.deepEqual(parseDiagnosticResult(state.result, 'luckycat-demo'), state.result);
    else assert.equal('result' in state, false);
  }
  const many = createPreviewState('many');
  assert.equal(
    many.kind === 'result' &&
      many.result.rules.reduce((count, rule) => count + rule.candidates.length, 0),
    50,
  );
});

for (const locale of ['ja', 'en'] as const)
  test(`all eight ${locale} states render without network access and preserve result meaning`, () => {
    const fetcher = vi.fn(() => {
    ...
    for (const scenario of previewScenarios) {
      ...
      assert.ok(
        screen.getByText(
          locale === 'ja' ? '架空データ · デザイン仮採用' : 'Fictional data · Provisional design',
        ),
      );
      const result = screen.queryByRole('region', {
        name: locale === 'ja' ? '診断結果' : 'Diagnostic results',
      });
      if (scenario.id === 'loading' || scenario.id === 'transport') {
        ...
        assert.equal(
          rows.length,
          scenario.id === 'many' ? 50 : ['empty', 'authentication'].includes(scenario.id) ? 0 : 3,
        );
          ...
          assert.ok(
            within(result).getByRole('heading', {
              name:
                locale === 'ja'
                  ? '一部を評価できませんでした'
                  : 'Some scopes could not be evaluated',
            }),
          );
        ...
        if (scenario.id === 'unknown') {
          assert.ok(
            within(result).getByText(
              locale === 'ja' ? '算出済み 0件 ／ 金額不明 3件' : '0 priced / 3 amounts unknown',
            ),
          );
        ...
        if (scenario.id === 'authentication') {
          ...
          assert.equal(
            within(result).queryByRole('heading', {
              name:
                locale === 'ja' ? '評価範囲内で該当なし' : 'No matches within the evaluated scope',
            }),
            null,
          );
        }
