...
test('attached and non-ready disks do not match; missing or unsupported evidence stays unevaluated', () => {
  const cases = [
    [{ referenceCount: 1 }, 'no_matching_candidates'], [{ state: 'CREATING' }, 'no_matching_candidates'],
    [{ referenceCount: null }, 'incomplete'], [{ state: null }, 'incomplete'], [{ state: 'NEW_STATE' }, 'incomplete'],
    [{ diskType: null }, 'incomplete'], [{ diskType: 'hyperdisk-balanced' }, 'incomplete'], [{ replicating: true }, 'incomplete'],
  ];
  for (const [change, conclusion] of cases) assert.equal(evaluateInventory(inventory('disks', [{ ...record, ...change }])).conclusion, conclusion);
});
test('only reserved external IPv4 without references matches; contradictions are not evaluated', () => {
  ...
  for (const [change, conclusion] of [
    [{}, 'candidates_found'], [{ addressType: 'INTERNAL' }, 'no_matching_candidates'], [{ ipVersion: 'IPV6' }, 'no_matching_candidates'],
    [{ state: 'IN_USE', referenceCount: 1 }, 'no_matching_candidates'], [{ state: 'RESERVING' }, 'no_matching_candidates'],
    [{ state: 'RESERVED', referenceCount: 1 }, 'incomplete'], [{ state: 'IN_USE' }, 'incomplete'],
    [{ ipVersion: null }, 'incomplete'], [{ addressType: null }, 'incomplete'], [{ referenceCount: null }, 'incomplete'],
  ]) assert.equal(evaluateInventory(inventory('addresses', [{ ...ip, ...change }])).conclusion, conclusion);
});
test('empty complete lists differ from failures and partial lists; valid partial findings remain visible', () => {
  ...
  for (const status of ['partial', 'failed', 'not_attempted']) {
    const result = evaluateInventory(inventory('disks', [], { status, issues: [{ reason: 'retrieval_failed' }] }));
    assert.equal(result.conclusion, 'incomplete'); assert.equal(result.status, 'not_evaluated');
  }
  const result = evaluateInventory(inventory('disks', [record, { ...record, resource: 'other', state: null }], { status: 'partial', issues: [{ reason: 'page_limit' }] }));
  assert.equal(result.status, 'partially_evaluated'); assert.equal(result.candidates.length, 1); assert.equal(result.unevaluated.length, 1);
