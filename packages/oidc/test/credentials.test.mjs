...
test('debug is opt-in, and malformed credentials cannot select an arbitrary token endpoint', () => {
  for (const mode of [undefined, 'production', 'wif'])
    assert.throws(() => createLocalServiceAccountProvider({ ...options, mode }), {
      message: 'disabled',
    });
  for (const value of [
    'invalid-secret',
    'null',
    '{"type":"api_key"}',
    JSON.stringify({ ...JSON.parse(credentials), token_uri: 'https://example.invalid/collect' }),
  ]) {
    assert.throws(
      () => createLocalServiceAccountProvider({ ...options, serviceAccountJson: value }),
...
test('signs a verifiable Google OAuth assertion with a read scope and no delegated subject', async () => {
  const provider = createLocalServiceAccountProvider({
    ...options,
    now: () => 1_000_000,
    fetcher: async (url, init) => {
      ...
      assert.deepEqual(JSON.parse(Buffer.from(payload, 'base64url')), {
        iss: 'reader@debug-example.iam.gserviceaccount.com',
        aud: url,
...
test('coalesces requests, refreshes before expiry, and isolates provider caches', async () => {
  ...
  await createLocalServiceAccountProvider(config).getAccessToken();
...
test('does not leak or cache failures and rejects malformed token responses', async () => {
  ...
  for (const result of [
    ...
    { access_token: 'secret', token_type: 'Bearer', expires_in: 7200 },
  ]) {
    const bad = createLocalServiceAccountProvider({
      ...options,
      fetcher: async () => Response.json(result),
    });
    await assert.rejects(bad.getAccessToken(), { message: 'token_exchange_failed' });
  }
  const denied = createLocalServiceAccountProvider({
    ...options,
    fetcher: async () => new Response('secret-must-not-escape', { status: 403 }),
  });
