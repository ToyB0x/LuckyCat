...
export function createLocalServiceAccountProvider(
  options: LocalCredentialOptions,
): AccessTokenProvider {
  ...
  async function exchange(): Promise<string> {
    ...
    try {
      ...
      const header = base64url(
        JSON.stringify({ alg: 'RS256', typ: 'JWT', ...(keyId ? { kid: keyId } : {}) }),
      );
